...
The Atlas MCP server provides MCP (Model Context Protocol) tools for path locking and trigger management. It runs as a stdio-based MCP server that Claude Code connects to via the `~/.atlas-mcp/.merged.json` config (generated by `claude-atlas`).
