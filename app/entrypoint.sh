...
chown -R agent:agent /home/agent /talos/logs
...
if [ -f "/home/agent/.index/talos.db" ]; then
  sqlite3 "/home/agent/.index/talos.db" "DELETE FROM path_locks;" 2>/dev/null || true
fi
...
export AGENT_NAME="${AGENT_NAME:-Talos}"

# Drop to agent user and start supervisord
# Pass PATH explicitly — sudo env_reset strips the Dockerfile ENV PATH otherwise
exec sudo -u agent \
  PATH="/talos/app/bin:/home/agent/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin" \
  HOME="/home/agent" \
  AGENT_NAME="$AGENT_NAME" \
  /usr/bin/supervisord -c /etc/supervisor/conf.d/talos.conf
